...
| Start anvil, rehearse on the fork, run `plan_diff` | Put the key and private RPC into `.env`; fund the account |
...
| **Live broadcast** | an explicit "yes" for this file in this session | fork rehearsal green, `plan_diff` exit 0, the pre-flight list in `04-deploy.md` §5 |
...
- Broadcast to a live chain without the three conditions: clean rehearsal, clean `plan_diff`, explicit "yes".
